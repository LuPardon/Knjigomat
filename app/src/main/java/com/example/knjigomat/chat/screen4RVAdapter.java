...
public class screen4RVAdapter extends RecyclerView.Adapter<screen4RVAdapter.screen4ViewHolder> implements Filterable {
    Context context;
    List<chat> chatList;
    List<chat> filteredList;
    fragment_screen4 fragment;

    public screen4RVAdapter(Context context, List<chat> chatList, fragment_screen4 fragment) {
    ...
    @Override
    public screen4RVAdapter.screen4ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(context).inflate(R.layout.chat_row, null, false);
        return new screen4ViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(screen4RVAdapter.screen4ViewHolder holder, int position) {
//        holder.name.setText(filteredList.get(position).get());
